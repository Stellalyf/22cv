...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        float d1 = 0, d2 = 0;
        for (i = 0; i < bn; i++) {
            d1 = l1_distance(a[j].data, b[bind].data, a[j].n);
            d2 = l1_distance(a[j].data, b[i].data, a[j].n);
            if (d1 > d2) {
                d1 = d2;
                bind = i;
            }
    ...
    for (i = 0; i < an; i++) {
        if (!seen[m[i].bi]) { seen[m[i].bi] = i + 1; }
        else {
...
float point_distance(point p, point q)
{
    ...
    float l2 = sqrt(pow(p.x - q.x, 2) + pow(p.y - q.y, 2));
    return l2;
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i=0; i < n; i++) { m[i].distance = point_distance(project_point(H, m[i].p), m[i].q); }
    qsort(m, n, sizeof(match), match_compare);
    for (i = 0; i < n; ++i, ++count) {
        if (m[i].distance > thresh)break;
    }
    return count;
}
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for (e = 0; e < k; e++) {
        ...
        int temp = model_inliners(H, m, n, thresh);
        if (best < temp) {
            H = compute_homography(m, temp);
            best = temp;
            Hb = H;
...
image combine_images(image a, image b, matrix H)
{
    ...
    for (i = 0; i < b.w; ++i) {
        for (j = 0; j < b.h; ++j) {
            point pro = project_point(H, make_point(i, j));
            if(pro.x>=0&&pro.x<b.w&&pro.y>=0&&pro.y<b.h)
            for (k = 0; k < b.c; ++k) {
                // TODO: fill in.
                set_pixel(c, i - dx, j - dy, k, bilinear_interpolate(b, pro.x, pro.y, k));
            }
        }
    }
...
image cylindrical_project(image im, float f)
{
    ...
    for (i = 0; i < im.w; i++)for (j = 0; j < im.h; j++) {
        ...
        if (x >= 0 && x < c.w && y >= 0 && y < c.h) {
            for (k = 0; k < c.c; k++)set_pixel(c, i, j, k, bilinear_interpolate(im, x, y, k));
        }
    }
    return c;
}
